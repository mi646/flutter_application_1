document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Set category as 'grocery' for all operations
    ...
    // Fetch Grocery Products Function
    async function fetchGroceryProducts() {
        try {
            ...
            if (products.length === 0) {
                productList.innerHTML = "<p>No grocery products found.</p>";
            }
    ...
    // Form Submit to Update Grocery Product
    updateProductForm.addEventListener('submit', async (event) => {
        ...
        try {
            await fetch(`http://localhost:3000/api/grocery/${productId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(product),
            });

            alert('Grocery Product Updated Successfully');
            fetchGroceryProducts();
            updateProductModal.style.display = 'none';
            updateProductForm.reset();
        } catch (err) {
            console.error(err);
            alert('Failed to Update Grocery Product');
        }
    });
    ...
    window.deleteProduct = async (id) => {
        if (confirm('Are you sure you want to delete this grocery product?')) {
            try {
                await fetch(`http://localhost:3000/api/grocery/${id}`, {
                    method: 'DELETE',
                });
                ...
                console.error(err);
                alert('Failed to Delete Grocery Product');
            }
        }
    };

    // Form Submit to Add Grocery Product
    ...
    // Load Grocery Products on Page Load
    fetchGroceryProducts();
});
