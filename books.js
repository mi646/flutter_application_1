document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Set category as 'books' for all operations
    ...
    // Fetch Books Products Function
    async function fetchBooksProducts() {
        try {
            ...
            if (products.length === 0) {
                productList.innerHTML = "<p>No books products found.</p>";
            }
            ...
            console.error('Failed to fetch books products:', err);
        }
    }

    // View & Update Book Product
    ...
    // Form Submit to Update Book Product
    updateProductForm.addEventListener('submit', async (event) => {
        ...
        try {
            await fetch(`http://localhost:3000/api/books/${productId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(product),
            });

            alert('Book Product Updated Successfully');
            fetchBooksProducts();
            updateProductModal.style.display = 'none';
            updateProductForm.reset();
        } catch (err) {
            console.error(err);
            alert('Failed to Update Book Product');
        }
    });

    // Delete Book Product
    window.deleteProduct = async (id) => {
        if (confirm('Are you sure you want to delete this book product?')) {
            try {
                await fetch(`http://localhost:3000/api/books/${id}`, {
                    method: 'DELETE',
                });

                alert('Book Product Deleted Successfully');
                fetchBooksProducts();
            } catch (err) {
                console.error(err);
    ...
    // Form Submit to Add Book Product
    addProductForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            console.error(err);
            alert('Failed to Add Book Product');
        }
    });
