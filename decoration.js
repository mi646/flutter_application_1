document.addEventListener("DOMContentLoaded", async () => {
    ...
    const category = "decoration"; // Category set to "beauty"
    ...
    async function fetchdecorationProducts() {
        try {
            const response = await fetch('http://localhost:3000/api/decoration');
            ...
            console.error('Failed to fetch decoration products:', err);
        }
    }
    ...
    addProductForm.addEventListener('submit', async (event) => {
        ...
        try {
            await fetch('http://localhost:3000/api/decoration', {
                method: 'POST',
                body: formData
            });
            ...
            addProductForm.reset();
            productImagePreview.src = '';
            fetchdecorationProducts(); // Refresh Beauty Products List
        } catch (err) {
            console.error(err);
    ...
    window.deleteProduct = async (id) => {
        if (confirm('Are you sure you want to delete this beauty product?')) {
            try {
                await fetch(`http://localhost:3000/api/decoration/${id}`, {
                    method: 'DELETE',
                });
    ...
    updateProductForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`http://localhost:3000/api/decoration/${id}`, {
                method: 'PUT',
                body: formData
            });
            ...
            updateProductForm.reset();
            updateImagePreview.src = '';
            fetchdecorationProducts(); // Refresh
        } catch (err) {
